-- Migration: Fix Service Orders RPCs
-- Date: 2024-12-20
-- Description: Drop and recreate service orders RPCs with correct field mappings

-- Drop existing functions first
DROP FUNCTION IF EXISTS get_service_order_details(UUID);
DROP FUNCTION IF EXISTS get_service_orders_stats(DATE, DATE);
DROP FUNCTION IF EXISTS update_service_order_status(UUID, service_order_status, TEXT);
DROP FUNCTION IF EXISTS search_service_orders(TEXT, service_order_status, service_order_priority, UUID, DATE, DATE, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS soft_delete_service_order(UUID);
DROP FUNCTION IF EXISTS restore_service_order(UUID);

-- Recreate get_service_orders_stats with correct status values
...
-- Recreate search_service_orders with correct parameter types
...
-- Recreate get_service_order_details with correct field mappings
...
-- Recreate update_service_order_status with correct parameter types
...
-- Recreate soft_delete_service_order
...
-- Recreate restore_service_order
