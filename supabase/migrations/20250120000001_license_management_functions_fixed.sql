-- Funções SQL para o Sistema de Gerenciamento de Licenças (Versão Corrigida)
-- Criado em: 2025-01-20
-- Descrição: Remove funções existentes e recria com novas assinaturas

-- Remover funções existentes que podem ter conflitos
DROP FUNCTION IF EXISTS admin_get_license_stats();
DROP FUNCTION IF EXISTS admin_get_all_licenses();
DROP FUNCTION IF EXISTS admin_create_license_advanced(text, timestamp with time zone, uuid, text, boolean);
DROP FUNCTION IF EXISTS admin_create_multiple_licenses(integer, timestamp with time zone, text, boolean);
DROP FUNCTION IF EXISTS admin_update_license(uuid, text, uuid, boolean, timestamp with time zone, text);
DROP FUNCTION IF EXISTS admin_extend_license(uuid, integer, text);
DROP FUNCTION IF EXISTS admin_transfer_license(uuid, uuid, text);
DROP FUNCTION IF EXISTS admin_get_license_history(uuid);
DROP FUNCTION IF EXISTS admin_bulk_license_action(uuid[], text, boolean, text);
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'license_history' AND table_schema = 'public') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'license_history' AND table_schema = 'public') THEN
    RETURN;
  END IF;
...
DROP POLICY IF EXISTS "Admins can view license history" ON public.license_history;
DROP POLICY IF EXISTS "Admins can insert license history" ON public.license_history;
