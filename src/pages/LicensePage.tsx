...
export const LicensePage = () => {
  ...
  const { licenseDetails } = useUserLicenseDetails();
  ...
  const calculateExpiredDays = () => {
    if (!licenseDetails?.expires_at) return 0;
    
    // Se há uma data de expiração, vamos calcular
    const expiredDate = new Date(licenseDetails.expires_at);
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20 flex items-center justify-center p-4">
      <div className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="lg:col-span-2 text-center space-y-2">
          ...
          {licenseData?.has_license && licenseDetails?.license_code && (
                ...
                {licenseDetails.expires_at && (
                  <div className="flex justify-center items-center gap-2">
                    ...
                    <span className="font-medium">
                      {new Date(licenseDetails.expires_at).toLocaleDateString('pt-BR')}
                    </span>
                  </div>
                )}
                {licenseDetails.activated_at && (
                  <div className="flex justify-center items-center gap-2">
                    <span className="text-muted-foreground">Ativada em:</span>
                    <span>
                      {new Date(licenseDetails.activated_at).toLocaleDateString('pt-BR')}
                    </span>
                  </div>
                )}
              </div>
